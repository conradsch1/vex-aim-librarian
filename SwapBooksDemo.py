...
``aim_librarian.shelf_slots`` (``slot_0``..``slot_3``; ``x = _TAG_X``; *y* in mm per slot
table). Requires ``install_librarian_extensions``.
...
class SwapBooksDemo(StateMachineProgram):
    ...
    Home and staging use fixed shelf *slot* indices 0--3 (``slot_0``..``slot_3``) with
    world-map centers from :func:`aim_librarian.shelf_slots.book_center_pose`.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for label, s in (
            ("home_slot_a", home_slot_a),
            ("home_slot_b", home_slot_b),
