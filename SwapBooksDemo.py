...
from math import nan, pi, radians
...
# Match ``CelesteLibrarian``: field shift (−3 cm X/Y); marker 16: X = −3 cm; Y and theta = marker 20.
...
# Match ``CelesteLibrarian.HOMING_DELTA_*``: corner robot start −1 cm in X, +14.5 cm in Y.
...
class SwapBooksDemo(StateMachineProgram):
    ...
    def __init__(self, book_id_a: int = BOOK_FIRST_ID, book_id_b: int = BOOK_FIRST_ID + 1, **kwargs):
        ...
        shelf_x = _TAG_X - 45.0 + SETUP_SHIFT_X_MM
        self._pose0_a = Pose(
            shelf_x + HOMING_DELTA_X_MM,
            15.0 + HOMING_DELTA_Y_MM + SETUP_SHIFT_Y_MM,
            z_book,
            pi,
        )  # book A's home slot
        self._pose0_b = Pose(
    ...
    def _pose_survey(self) -> Pose:
        ...
        shelf_x = _TAG_X - 45.0 + SETUP_SHIFT_X_MM
        return Pose(
            shelf_x - 185.0 + HOMING_DELTA_X_MM,
            HOMING_DELTA_Y_MM + SETUP_SHIFT_Y_MM,
            0.0,
            0.0,
        )
