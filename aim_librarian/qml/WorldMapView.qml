...
View3D {
    ...
    function debugSnapshot() {
        ...
        for (let row = 0; row < count; ++row) {
            ...
            } else if (type === "apriltag" || type === "aruco" || type === "book") {
    ...
    Component {
        ...
        Node {
            ...
            // Book: X=spine thickness, Y=cover width, Z=height; ArUco is on the spine (±Y tall face)
            ...
            readonly property bool tagOnSpine: model.type === "book"
            // Use model.z which is already computed in Python (height/2 = 24mm)
            position: Qt.vector3d(model.x, model.y, model.z)
            // Books: +90° yaw vs pose so prism/spine align with expected shelf framing (visual only).
            eulerRotation: Qt.vector3d(0, 0, radiansToDegrees(model.theta) + (tagOnSpine ? 90 : 0))
            ...
            Model {
                ...
                materials: PrincipledMaterial {
                    baseColor: {
                        ...
                        if (model.type === "book")
                            return model.visible ? "#6b4423" : "#3d2615";
                        return "#202020";
                    }
                    emissiveFactor: {
                        if (!model.visible)
                        ...
                        if (model.type === "book")
                            return Qt.vector3d(0.1, 0.06, 0.02);
            ...
            Image {
                id: tagImageFront
                source: (model.type === "apriltag" || model.type === "aruco" || model.type === "book")
                        ? "image://tagtexture/" + ((model.type === "aruco" || model.type === "book") ? "aruco-" : "") + String(model.marker_id)
                        : ""
                visible: false
                cache: true
            }
            ...
            Image {
                id: tagImageBack
                source: (model.type === "apriltag" || model.type === "aruco" || model.type === "book")
                        ? "image://tagtexture/back-" + ((model.type === "aruco" || model.type === "book") ? "aruco-" : "") + String(model.marker_id)
                        : ""
                visible: false
                cache: true
            }
            
            // Tag panel — flat markers: outward +X / -X. Books: ArUco on spine faces ±Y (XZ plane, tall × thin).
            Model {
                id: textPanelFront
                visible: (model.type === "apriltag" || model.type === "aruco" || model.type === "book")
                         && model.marker_id !== null && model.marker_id !== undefined && model.marker_id !== ""
                source: "#Cube"
                scale: tagOnSpine
                       ? Qt.vector3d(thicknessMm / 100, 0.005, heightMm / 100)
                       : Qt.vector3d(0.005, heightMm / 100, widthMm / 100)
                position: tagOnSpine
                          ? Qt.vector3d(0, widthMm / 2 + 0.5, 0)
                          : Qt.vector3d(thicknessMm / 2 + 0.5, 0, 0)
                eulerRotation: tagOnSpine ? Qt.vector3d(0, 0, 0) : Qt.vector3d(90, 0, 0)
            ...
            Model {
                ...
                position: tagOnSpine
                          ? Qt.vector3d(0, -widthMm / 2 - 0.5, 0)
                          : Qt.vector3d(-thicknessMm / 2 - 0.5, 0, 0)
                eulerRotation: tagOnSpine ? Qt.vector3d(0, 180, 0) : Qt.vector3d(90, 0, 0)
